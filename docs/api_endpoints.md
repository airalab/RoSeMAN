...
| Method | Path                           | Handler                   | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------ | ------------------------------ | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/v3/messages/json`        | `getMessagesJson()`       | Valid/verified Connectivity Protocol records with `message_json` materialized during indexing. All query parameters are optional: independent Unix-millisecond `start` and `end` bounds, `limit` (1–1000, default 1000), opaque 28-character `cursor`, `sensor_id`, `owner`, `payload_type`, `measurement_type`. There is no maximum date range; when both bounds are supplied, `start` must be less than `end`. Pass `next_cursor` unchanged as `cursor` with the same filters to read the next page; `null` marks the last page. Response: `{ result: { items, next_cursor } }`; every item contains `sensorId`, decimal-string `timestamp`, and decoded `core.v1.Message`. Top-level `nonce` and `signature` are omitted. |
| GET    | `/api/v3/messages/latest/json` | `getLatestMessagesJson()` | The newest valid/verified Connectivity Protocol message for every sensor that has data in the required `[start, end)` range of at most 24 hours. Optional filters: `sensor_id`, `owner`, `payload_type`, `measurement_type`. No `limit`, `cursor` or `next_cursor`; item format is identical to `/api/v3/messages/json`.                                                                                                                                                                                                                                                                                                                                                                                                     |

### Connectivity protobuf

| Method | Endpoint                  | Handler                       | Description                                                                                                                                                                                                                                          |
| ------ | ------------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/v3/messages`        | `getMessagesProtobuf()`       | Parameters and selection rules match `/api/v3/messages/json`. Returns a binary `crypto.v1.SignedEnvelopeBatch` with `Content-Type: application/protobuf`. The next-page cursor is returned in `X-Next-Cursor`; an absent header marks the last page. |
| GET    | `/api/v3/messages/latest` | `getLatestMessagesProtobuf()` | Parameters and selection rules match `/api/v3/messages/latest/json`. Returns a binary `crypto.v1.SignedEnvelopeBatch` without pagination.                                                                                                            |

See [api.md](./api.md#connectivity-protobuf-responses) for a decoding example.

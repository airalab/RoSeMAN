...
export default {
  ...
  async listUrban(req, res) {
    ...
    try {
      const rows = await getListSensorsUrban(start, end);
      const owners = await getOwnerBySensorsV2(
        rows.map((item) => item.sensor_id)
      );
      const result = rows.map((item) => {
  ...
  async sensor(req, res) {
    ...
    try {
      const result_sensor = await getBySensor(sensor, start, end);
      ...
      const subscriptionAccounts = await Subscription.find({
        owner: owner,
      }).lean();

      const result = {};
      const sensors = subscriptionAccounts.map((item) => item.account);
      for (const account of sensors) {
        if (account !== sensor) {
          result[account] = await getBySensor(account, start, end);
        }
      }
